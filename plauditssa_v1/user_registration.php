<?php
    ...
    $user_check_query = "SELECT user_email FROM users WHERE user_email = '$user_email' LIMIT 1";
    ...
    $row = mysqli_fetch_assoc($result);
        ...
        if($user) {
        	...
        	$hashPassword = password_hash($password, PASSWORD_DEFAULT);
          	$user_id = $row['user_id'];
        	$query = "UPDATE users SET user_firstName = '$user_firstName', user_lastName = '$user_lastName', user_password ='$hashPassword', verification_code = '' WHERE userID = $user_id";
...
<head>
	...
	<title>Plaudits SA - User Verification</title>
							...
							<div class="cart_extra_content cart_extra_total">
								<div class="checkout_title">Sign Up</div><br>
							 	...
							 	<?php
							 		...
							 		$sql = "SELECT * FROM users WHERE user_email = '$user_email' AND verification_code = '$verification_code'";
								...
								<div class="checkout_form_container">
									<form action="" method="post" id="checkout_form" class="checkout_form">
										<div class="row">
											<div class="col-lg-6">
												<!-- Name -->
												<input type="text" name="firstName" id="checkout_name" class="checkout_input" placeholder="First Name" required="required" value="<?php echo$row['user_firstName'] ?>">
											</div>
											<div class="col-lg-6">
												<!-- Last Name -->
												<input type="text" name="lastName" id="checkout_last_name" class="checkout_input" placeholder="Last Name" required="required" value="<?php echo$row['user_lastName'] ?>">
											</div>
										</div>
										<div>
											<!-- Email -->
											<input type="email" name="email" id="checkout_email" class="checkout_input" placeholder="Email" required="required" pattern="[A-z0-9.]+@[A-z]+\.[A-z.]+" value="<?php echo$row['user_email'] ?>" readonly>
										</div>
										<div>
											<!-- Phone no -->
											<input type="tel" name="phone" id="checkout_phone" class="checkout_input" placeholder="Phone No." required="required" pattern="[0-9]{10}" value="<?php echo$row['user_phone'] ?>">
										</div>
										<div>
											<!-- Password -->
											<input type="password" name="password" id="password" class="checkout_input" placeholder="Password" required="required" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}">
										</div>
										<div>
											<!-- Password -->
											<input type="password" name="password2" id="password2" class="checkout_input" placeholder="Confirm Password" required="required" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}">
										</div>
