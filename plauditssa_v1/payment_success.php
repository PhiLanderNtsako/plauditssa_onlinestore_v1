<?php
    ...
    $preOrder_id = preg_replace('/[^0-9]/', '', ''.$_SESSION['rdurl'].'');
	$sql = "UPDATE post_orders SET postOrder_paymentStatus = 'Paid' WHERE preOrderID = $preOrder_id";
	$query = $conn->query($sql);
	echo mysqli_error($conn);

	$query="SELECT * FROM pre_orders, users WHERE pre_orders.userID = users.userID AND pre_orders.preOrderID = '$preOrder_id'";
	...
	$subject = "Payment for Order PLDTS#$preOrder_id Recieved";
			
	$message = '<h2>Payment for order PLDTS#$preOrder_id was Successfully Processed</h2><br><br>

				<p><h4>Hi '.$row['user_firstName'].' '.$row['user_lastName'].',</h4></p><br>

				<p>Your payment for the following order: PLDTS#'.$preOrder_id.'</p>
				<p>Was Successfully Recieved. View You Order On the Link Below.</p>
				<hr><br>
...
<head>
	...
	<title>Plaudits SA - Payment Successful</title>
		...
		<div class="checkout">
			<div class="container">
				<div class="row">
					...
					<div class="col-lg-6">
						<div class="billing">
							<div class="checkout_title"><h2>Payment Successfully Processed</h2></div>
	...
	<?php 
		if(isset($_SESSION['userID'])) {
            ...
            while($row=mysqli_fetch_array($result)) {
				...
				echo'<meta http-equiv="refresh" content="5; url= https://plauditssa.co.za/user-account/">';
        	}
        } else {
